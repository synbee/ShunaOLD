	...
	handle = async (event: IEvent): Promise<void> => {
		...
		if (bye) {
			const goodbye = await new Canvas.Goodbye()
				...
				.setGuildName(group.subject)
				.setAvatar(pfp)
				.setColor("border", "#27B6AF")
				.setColor("username-box", "#27B6AF")
				.setColor("discriminator-box", "#27B6AF")
				.setColor("message-box", "#27B6AF")
				.setColor("title", "#27B6AF")
				.setColor("avatar", "#27B6AF")
				.setText("member-count", `- ${memberCount} member !`)
